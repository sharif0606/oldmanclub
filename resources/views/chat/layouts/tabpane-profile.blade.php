...
<div>
    ...
    <div class="text-center p-4 border-bottom">
        <div class="mb-4">
            @if ($client->image)
            <img src="{{ asset('public/uploads/client/' . $client->image) }}" class="rounded-circle avatar-lg img-thumbnail imgavatar" alt="">
            @else
            <img src="{{ asset('public/images/download.jpg') }}" class="rounded-circle avatar-lg img-thumbnail imgavatar" alt="">
            @endif
        </div>

        <h5 class="font-size-16 mb-1 text-truncate profile-newname">{{($client->display_name)}}</h5>
        @if(Cache::has('is_online' . currentUserId()))
                ...
                <div id="profile-user-collapseOne" class="collapse show"
                    ...
                    <div class="card-body">
                        <div>
                            ...
                            <h5 class="font-size-14 profile-newname text-truncate">{{($client->display_name)}}</h5>
                        </div>
                        <div class="mt-4">
                            <p class="text-muted mb-1">{{ __("Email") }}</p>
                            <h5 class="font-size-14">{{($client->email)}}</h5>
                        </div>
                        <div class="mt-4">
                            <p class="text-muted mb-1">{{ __("Time") }}</p>
                            <h5 class="font-size-14">{{($client->created_at)}}</h5>
                        </div>
                        <div class="mt-4">
                            <p class="text-muted mb-1">{{ __("Location") }}</p>
                            <h5 class="font-size-14 mb-0">{{($client->current_country_id)}}</h5>
                        </div>
                    </div>
                </div>
            </div>
